...
export const InjectionView: React.FC<InjectionViewProps> = ({ libraryName, approach, bindings }) => {
  ...
  return (
    <div style={{ padding: 32, maxWidth: 960, fontFamily: 'var(--font-body-regular-family)', color: 'var(--color-text-main)' }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <div style={{ padding: '8px 14px', borderRadius: 8, border: '1px solid var(--color-neutral-200)', background: 'var(--color-surface)', textAlign: 'center' }}>
          <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--color-brand-500)' }}>{bindings.length}</div>
          <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Bindings</div>
        </div>
        <div style={{ padding: '8px 14px', borderRadius: 8, border: '1px solid var(--color-neutral-200)', background: 'var(--color-surface)', textAlign: 'center' }}>
          <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--color-brand-500)' }}>{categories.length}</div>
          <div style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Categories</div>
        </div>
      </div>
      ...
      {grouped.map(({ cat, items }) => (
        <div key={cat} style={{ marginBottom: 24 }}>
          ...
          <div style={{
            ...
            fontFamily: 'var(--font-code-family)', fontSize: 13,
          }}>
            {items.map((b, i) => (
              <div key={i} style={{
                padding: '6px 16px',
